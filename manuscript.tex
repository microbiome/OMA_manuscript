...
for multi-modal data and considerable reductions in execution time and
memory allocation (Figure~\ref{fig-benchmarking}). Conversion functions
and detailed cheat sheets between the two formats are available to
...
\includegraphics[width=1\textwidth,height=\textheight]{figures/TreeSE.png}
...
\includegraphics[width=0.6\textwidth,height=\textheight]{figures/mae.png}
...
\caption{\label{fig-benchmarking}\textbf{Benchmarking mia/TreeSE against
phyloseq and speedyseq.} TreeSE, phyloseq and speedyseq were benchmarked
on five common analytical operations using random subsets of samples
from the GrieneisenTSData experiment \citep{Grieneisen2021}. Execution
time (s) values and error bars represent the mean and standard error
(SE) over 10 iterations, respectively. Allocated memory (MB) was
...
phyloseq, could also be performed on a regular laptop (e.g., 4 CPU cores
and 16 GB RAM).}
...
framework for microbiome analysis and direct data import into the
...
include relative abundance \citep{gloor2017}, centered log-ratio (CLR)
and its robust version (rCLR), which are used to mitigate
...
\textbf{Differential abundance analysis (DAA).} DAA methods are used to
...
2,300 actively maintained software packages, of which 53 are classified
...
The mia contributors: \textbf{we will list here main and consortium
authors who approved their inclusion} on
