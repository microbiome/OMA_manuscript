...
be seldom studied in isolation. The challenge for a data scientist is
that different methods often require variable and inconsistent input
data formats, leading to technical complications and time lost on
repeated data wrangling. Bioconductor addresses this through
standardized data structures supported by an ecosystem of interoperable
methods, allowing more time to be invested in core analytical tasks.
...
to similar data integration tasks, notably in single-cell and spatial
omics through the SingleCellExperiment (SCE) \citep{amezquita2020} and
SpatialExperiment \citep{righelli2022} classes, respectively.
...
datasets by incorporating feature and sample organisations as tree
...
workflows. The direct interoperability with the widely adopted SE data
science ecosystem distinguishes our TreeSE-based approach from phyloseq,
which is another popular Bioconductor data container originally
developed with a focus on 16S amplicon data analysis
\citep{mcmurdie2012}. TreeSE is a superset that can accommodate all
elements of a phyloseq object, while also offering improved support for
multi-modal data and often substantial gains in memory efficiency and
computing speed (Figure~\ref{fig-benchmarking}). Overall, the TreeSE
...
Flexible linking of samples across several data objects supports
...
representing, for example, taxonomic or functional profiles. Taxonomic
abundance tables are one of the most commonly encountered data types.
Common measurement platforms include amplicon-based marker gene studies
...
require additional data wrangling. To streamline data import, we provide
importers for a variety of standard microbiome file formats including
BIOM \citep{mcdonald2012}, MetaPhlAn/HUMAnN (MetaPhlAn v. 2-4, HUMAnN v.
3) \citep{mciver2017}, Mothur \citep{schloss2009}, QIIME2
...
bridge the gap between raw sequence processing and statistical analysis
in R, allowing researchers to focus on interpreting biological patterns
rather than wrangling file formats. Moreover, converters between
alternative data structures in R are available supporting seamless
conversions between TreeSE and BIOM, DADA2 \citep{callahan2016} and
phyloseq (the mia::convert* functions), and TreeSE-based microbiome data
objects can be exported into external services such as MicrobiomeDB
\citep{Oliveira2018} and in other languages (\emph{e.g.} Julia). These
bridges facilitate the broadest possible access to microbiome analysis
methods.
...
comparisons the data are often subsetted, filtered or agglomerated to
higher-level taxonomic or functional categories or stratified by sample
groups.he package ecosystem and the mia package feature various tools
...
(\emph{e.g.} butyrate production \citep{Kullberg2024}, gut-brain modules
...
depths and it remains widely debated in microbial ecology
...
scattered along the workflow -- one can ensure well-organized and
...
\includegraphics[width=\linewidth,height=0.55\textheight,keepaspectratio]{figures/workflow.png}

}

\caption{\label{fig-workflow}\textbf{OMA workflow}. After data
generation and pre-processing (top two panels), the OMA workflow
provides tools for five steps of microbiome data analysis: 1) Data is
wrapped into a TreeSummarizedExperiment data object, containing
information on the samples (tube icons) and their features, such as ASVs
or other taxonomic units (colored circles). TreeSummarizedExperiment
objects consist of i) ``assays'' that describe the abundances of
features in each sample, ii) ``colData'' containing metadata on each
sample, iii) ``rowData'' containing taxonomy of each feature and iv) the
phylogenetic tree among features. Unlike phyloseq,
TreeSummarizedExperiment objects accommodate multiple assay tables as
well as additional hierarchical structures. 2) The
TreeSummarizedExperiment data object can then be processed to get
cleaner data. This can include filtering out low-quality samples and
features, transforming assay data for example by normalizing it to make
sample-specific abundances comparable, or agglomerating the feature data
to a specific taxonomic level. 3) The clean data can now be analysed
using statistical models to reveal compositional clustering patterns
among samples (e.g., with PCoA), analyse drivers of variation in sample
alpha and beta diversity or test for differential abundance of features
across sample variables (e.g., sampling time or location). 4) The OMA
workflow also allows integration of various different types of data
using MultiAssayExperiment. Here, for example, the taxonomic and
metabolic profiles of samples can be summarized and compared and
cross-association maps among features (e.g.~taxa or metabolites) can be
constructed. 5) Finally, OMA workflow supports reporting of analysis
using for example Quarto, to aid reproducible sharing of results.}

\end{figure}%

\section{Downstream data analysis}\label{sec-analysis}

The new Bioconductor framework for microbiome data science improves the
...
data formats \citep{quince2017}. These and other unique characteristics
have wide-reaching implications for microbiome analysis, necessating
dedicated analysis methods \citep{moreno-indias2021}. Our statistical
programming framework supports the community-driven development and
benchmarking of the solutions. The following sections provide an
...
Phylogenetic Diversity can accelerate the performance by 1-2 orders of
...
common indices in microbial ecology including Bray-Curtis, Jaccard,
...
(\emph{e.g.} Permutational Multivariate Analysis of Variance) and
...
covariates. This can benefit from compositionally robust measures
(\emph{e.g.} robust Aitchison distance \citep{Martino2019}), or
...
provided for instance by mia, miaViz, and additional single-cell
packages (scater, scuttle \citep{amezquita2020}).
...
methods are used to identify microbial taxa whose abundances vary across
study groups or continuous gradients
...
us and others indicated that elementary methods, for example log
transformed total sum scaled (TSS) counts coupled with a linear model,
or presence/absence data with logistic regression often yield the most
consistent results \citep{Pelto2025}. The standard DAA typically focuses
on individual features, ignoring interactions and potential co-abundance
...
classifications or functional properties (\emph{e.g.} butyrate producers
\citep{Kullberg2024}). Beyond these, advanced methods---such as causal
mediation analysis tailored to microbiome multi-omics data---are already
being developed using SummarizedExperiment \citep{jiang2025},
highlighting its readiness to support emerging innovations in microbiome
analytics.
...
metagenomic features, \emph{i.e.} antimicrobial resistance gene
composition, diversity, and load \citep{Salehi2025}. We have converted a
large-scale compilation of resistome profiles in 8,972 metagenome
samples \citep{Lee2023} in the TreeSE format for research and
...
miaTime R package offers dedicated solutions for longitudinal microbiome
studies. In addition, Bioconductor provides methods from other
application domains, among which single-cell sequencing
...
can be incorporated as a covariate or by estimating temporally or
spatially resolved metrics, for instance divergence, auto-correlation
and modality. Another type of temporal analysis includes the
time-to-event, or survival models, which are encountered in, for
example, epidemiological and population cohort studies; specialized
survival analysis strategies for microbiome data have been proposed
...
including the Hubbell's neutral model, generalized Lotka-Volterra, and
