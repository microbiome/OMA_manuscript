...
\caption{\label{fig-datacontainers}Bioconductor data containers for
microbiome data. \textbf{(a) TreeSummarizedExperiment (TreeSE).} TreeSE
supports hierarchical and multi-table analysis in microbiome data
science \citep{huang2021}. It links the numeric abundance matrix and
potential alternative versions (statistical transformations, different
taxonomic levels, dimensionality reduction) with tabular and
hierarchical side information on the features (rows) and samples
(columns). Additional slots for reference sequences and experiment
metadata are available. (Figure modified from
...
pipelines pose different biases and generate varying data formats
...
functional properties (e.g.~butyrate producers \citep{Kullberg2024}).
Beyond these, advanced methods---such as causal mediation analysis
tailored to microbiome multi-omics data---are already being developed
using SummarizedExperiment \citep{jiang2025}, highlighting its readiness
to support emerging innovations in microbiome analytics.
...
\citep{Velten2022}, and real, pseudo, or simulated time series. The
miaTime package supports longitudinal microbiome analyses. Time series
from standard models of microbial community dynamics including e.g.~the
Hubbell's neutral model, generalized Lotka-Volterra, and
consumer-resource model can be simulated with the miaSim package
\citep{gao2023}. Moreover, an increasing number of microbiome studies
are incorporating the spatial dimension \citep{Ruuskanen2021}. The
compatibility between data structures support the adotion of solution
from from other application domains in Bioconductor, such as single-cell
sequencing \citep{amezquita2020} and spatial transcriptomics
\citep{righelli2022} (Figure~\ref{fig-downstream_analyses}). Spatial and
...
e.g.~epidemiological and population cohort studies; specialized survival
analysis strategies for microbiome data have been proposed
\citep{Salosensaari2021, Calle2023}.
...
greatly expand the selection of available toolkits in emerging areas. In
...
OMA consortium: Thomaz Bastiaanssen, Moritz E. Beber, Aituar Bektanov,
Giulio Benedetti, Tuomas Borman, Domenick J. Braccia, Héctor Corrada
Bravo, Matteo Calgaro, Danielle Callan, Yang Cao, Jiya Chaudhary, Sneha
Das, Basil Courbayre Dussau, Henrik Eckermann, Pande Erawijantari, Felix
M. Ernst, Elliot Gaudron-Parry, Maija Hakula, Kati Hanhineva, Aki
Havulinna, Rasmus Hindström, Ruizhu Huang, Akewak Jeba, Leo Lahti, Himmi
Lindgren, Yihan Liu, Himel Mallick, Dattatray Mongad, Christian L.
Müller, Geraldson Muluh, Signe Nynäs, Jesse Pasanen, Joao Paulo, Juho
Pelto, Stefanie Peschel, Renuka Potbhare, Théotime Pralas, Marcel Ramos,
Aura Raulo, Artur Sannikov, Ely Seraidarian, Jacques Serizay, Sudarshan
Shetty, Rajesh Shigdel, Vilhelm Suksi, Eetu Tammi, Benjamin Valderamma,
Levi Waldron, Lu Yang.
