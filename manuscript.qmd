...
of digital resources, active user communities and collaboration culture that
...
workflows for transcriptomics [@crowell2025], single-cell [@amezquita2020],
...
supporting the community-driven development
and integration within the broader Bioconductor ecosystem. To promote
...
framework. In contrast, Bioconductor spans multiple areas of bioinformatics also
...
open community, resulting in a broad toolset and collaboration enabling easy
reuse of common solutions (see the online book for a
comprehensive overview of packages supporting the microbiome data science
ecosystem).
Although not without its own challenges, such as dependency management,
...
TreeSE is a more general framework than phyloseq, as it can
...
other profiles and alternative feature representations required for contemporary
microbiome research. In addition, TreeSE offers
...
scales more efficiently in terms of execution time (t) and allocated
memory (m) as sample size (n) increases. Built-in support
for sparse and delayed matrices in SE-based data containers, can improve
performance even further [@huber2015].
To facilitate migration from phyloseq, the OMA online book provides
...
analysis pipelines. Despite the advantages of the TreeSE-based framework,
...
such a systematic framework that enforces anchor alignment. This build upon lessons
...
| A compilation of open microbiome data sets | [https://github.com/microbiome/data](https://github.com/microbiome/data) | 8                     | 18,777    |
...
(*e.g.*, Permutational Multivariate Analysis of Variance, PERMANOVA) and supervised
...
saveRDS(pkg_stats, file.path("data", "pkg_stats.Rds"))
```

```{r}
#| label: bioc_views2
pkg_stats <- readRDS(file.path("data", "pkg_stats.Rds"))
```
...
Teo Dallier; Noah de Gunst; Karoline Faust; Robert D. Finn; Samuel Hillman; Ruizhu
