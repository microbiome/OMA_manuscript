...
abstract: |
  ...
  of modular data science workflows through interoperable methods. Building on
  ...
  we introduce a community-developed open source ecosystem
  ...
  community, and supports standardized and reproducible data
...
evidence-based best practices, we introduce the online book "Orchestrating
Microbiome Analysis with Bioconductor" (OMA), which represents a comprehensive
compilation of current practices and challenges in microbiome analysis using the
...
framework. In contrast, Bioconductor supports a more exploratory, statistically
...
In particular, the SE-based framework has been extended to
...
storage of other common components of microbiome data, for example, reference 
...
TreeSE provides a more general framework than phyloseq, as the former can
accommodate not only the typical components of a phyloseq object
...
time and memory consumption represents a key area for further development
in the TreeSE-based framework. To promote interoperability between the two
respective user communities, TreeSE-phyloseq conversion tables and detailed
cheat sheets are available in the OMA book.
...
ability to incorporate multiple data modalities using different data containers.
...
| A compilation of open microbiome datasets  | https://github.com/microbiome/data        | 8                     | 18,777    |
...
wrapped into a TreeSE data object (@fig-datacontainers),
...
unsupervised analyses, and help to uncover and quantify covariation
...
standardized methods supporting the integrative data containers of the SE framework.
...
methods in microbiome research support the SE framework, including
ANCOM-BC2 [@lin2024], LimROTS [@anwar2025], MaAsLin3
[@nickols2024], and radEmu [@clausen2024]. Leveraging the interoperability of
the SE framework, several of these methods have also found
applications with other omics modalities, such as transcriptomics
(*e.g.* DESeq2), metabolomics [@nickols2024], or proteomics [@anwar2025].
Importantly, recent studies have indicated that elementary methods, such as log
...
significantly relies on strong community engagement, which
...
transparency, interpretability, and understanding of the data, while expressive
code and meaningful comments further support traceability and verification.
...
fostering community-driven consensus, while also allowing users to flexibly select
...
interactive applications, these resources offer accessible entry points for
developers and users, regardless of their background.
...
that provides a comprehensive guide to the microbiome
data science framework in Bioconductor, including practical guidelines,
...
(OSCA book [@amezquita2020]) and transciptomics (OSTA book [@crowell2025]).
These resources
...
To achieve a trade-off between automation and flexibility, data scientists
often recombine open-source software components
into custom workflows. However, the lack of commonly adopted
...
an R/Bioconductor ecosystem for multi-modal microbiome data science. Building on
previously established data structures [@huang2021; @ramos2017],
we introduce a comprehensive, interoperable ecosystem
developed as an extensive community effort. We also provide exhaustive guidance in the
form of the OMA book,
thereby supporting efficient learning and reproducible analysis. This ultimately results
in a readily usable ecosystem designed for microbiome research.
