...
if( TRUE || is.null(df) || nrow(pkgs) == 0L ){
...
benchmark_df %>%
  ...
  write.csv(file = "data/benchmark_results.csv", row.names = FALSE)
```
...
microbiomeDataSets package in Bioconductor). Execution time (s) was measured as
the mean value over 10 iterations, whereas allocated memory (MB) was estimated based on a single iteration.
